...
void ver_cmd()//verify if command can be executed
{
...
void wrt(string in) // write to file and execute command
{
string ch="chmod +x ";

const char *pwd="/tmp/sc";

fstream fs;
fs.open(pwd,fstream::out);

fs.write(beg,strlen(beg));
fs.write("\n",2);
...
if(srt!=tr.gt_str())
{
std::cout<<"some error in programm"<<"\n";
std::cout<<tr.gt_str()<<" is not equal to "<<srt;
